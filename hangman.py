...
assert_equal(
 ...
 Page(state=State(name='', word='', guessed_letters=[], wrong_guesses=0, previous_games=[]),
     content=['Welcome to Hangman!',
              Image(url='title_screen.png', width=None, height=None),
...
assert_equal(
 ...
 Page(state=State(name='', word='', guessed_letters=[], wrong_guesses=0, previous_games=[]),
     content=['Please enter your name:',
              TextBox(name='name', kind='text', default_value=None),
              Button(text='Play!', url='/initialize_game')]))

assert_equal(
 check_guess(State(name='Babbage', word='HAMBURGER', guessed_letters=['A', 'B', 'E', 'S'], wrong_guesses=1, previous_games=[]), 'z'),
 Page(state=State(name='Babbage',
                 word='HAMBURGER',
                 guessed_letters=['A', 'B', 'E', 'S', 'Z'],
                 wrong_guesses=2,
                 previous_games=[]),
     content=['Guess a Letter!',
              Image(url='hangman2.png', width=None, height=None),
              '_ A _ B _ _ _ E _',
              TextBox(name='guess', kind='text', default_value=''),
              Button(text='Go', url='/check_guess'),
              'Guessed Letters: A, B, E, S, Z']))

assert_equal(
 check_guess(State(name='Babbage', word='HAMBURGER', guessed_letters=['A', 'B', 'E', 'P', 'Q', 'S', 'Z'], wrong_guesses=4, previous_games=[]), 't'),
 Page(state=State(name='Babbage',
                 word='HAMBURGER',
                 guessed_letters=['A', 'B', 'E', 'P', 'Q', 'S', 'T', 'Z'],
                 wrong_guesses=5,
                 previous_games=[]),
     content=['Guess a Letter!',
              Image(url='hangman5.png', width=None, height=None),
              '_ A _ B _ _ _ E _',
              TextBox(name='guess', kind='text', default_value=None),
              Button(text='Go', url='/check_guess'),
              'Guessed Letters: A, B, E, P, Q, S, T, Z']))

assert_equal(
 check_guess(State(name='Babbage', word='HAMBURGER', guessed_letters=['A', 'B', 'E', 'P', 'Q', 'S', 'T', 'Z'], wrong_guesses=5, previous_games=[]), 'j'),
 Page(state=State(name='Babbage',
                 word='HAMBURGER',
                 guessed_letters=['A', 'B', 'E', 'J', 'P', 'Q', 'S', 'T', 'Z'],
                 wrong_guesses=6,
                 previous_games=[]),
     content=['You lost! The word was HAMBURGER',
              Image(url='lose.png', width=None, height=None),
              Button(text='Main Menu', url='/reset')]))

assert_equal(
 reset(State(name='Babbage', word='HAMBURGER', guessed_letters=['A', 'B', 'E', 'J', 'P', 'Q', 'S', 'T', 'Z'], wrong_guesses=6, previous_games=[])),
 Page(state=State(name='Babbage', word='', guessed_letters=[], wrong_guesses=0, previous_games=[]),
     content=['Welcome to Hangman!',
              Image(url='title_screen.png', width=None, height=None),
              Button(text='New Game', url='/new_game')]))

assert_equal(
 check_guess(State(name='Babbage', word='PYTHON', guessed_letters=['E', 'H', 'O', 'P', 'T', 'Y'], wrong_guesses=1, previous_games=[]), 'n'),
 Page(state=State(name='Babbage',
                 word='PYTHON',
                 guessed_letters=['E', 'H', 'N', 'O', 'P', 'T', 'Y'],
                 wrong_guesses=1,
                 previous_games=[]),
     content=['You won! The word was PYTHON',
              Image(url='win.png', width=None, height=None),
              Button(text='Main Menu', url='/reset')]))
